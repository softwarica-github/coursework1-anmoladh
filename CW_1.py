...
class Stegno:
    ...
    def main(self, root):
        ...
        f = Frame(root, bg="#F0F0F0")  # Set background color

        title = Label(f, text='Image Steganography', bg="#F0F0F0", fg="#333333")
        title.config(font=('Arial', 24, 'bold'))
        title.grid(pady=20)

        encode_button = Button(f, text="Encode", command=lambda: self.frame1_encode(f), padx=20, bg="#66BB6A", fg="white")  # Green button
        encode_button.config(font=('Arial', 14))
        decode_button = Button(f, text="Decode", padx=20, command=lambda: self.frame1_decode(f), bg="#42A5F5", fg="white")  # Blue button
        decode_button.config(font=('Arial', 14))
        encode_button.grid(pady=20)
        decode_button.grid()

        f.grid(row=1, column=1, padx=50, pady=50)
    ...
    def frame1_decode(self, f):
        ...
        l1.grid()
        select_button = Button(d_f2, text='Select', command=lambda: self.frame2_decode(d_f2))
        select_button.config(font=('Arial', 14))
        select_button.grid(pady=20)
        cancel_button = Button(d_f2, text='Cancel', command=lambda: Stegno.home(self, d_f2))
        cancel_button.config(font=('Arial', 14))
        cancel_button.grid(pady=10)
        d_f2.grid()

    def frame2_decode(self, d_f2):
        d_f3 = Frame(root)
        myfile = tkinter.filedialog.askopenfilename(filetypes=[('Image Files', '*.png *.jpeg *.jpg'), ('All Files', '*.*')])
        if not myfile:
            ...
            label_selected = Label(d_f3, text='Selected Image:')
            label_selected.config(font=('Arial', 18))
            label_selected.grid()
            panel = Label(d_f3, image=img)
            panel.image = img
            panel.grid()
            hidden_data = self.decode(myimg)
            label_hidden = Label(d_f3, text='Hidden Data:')
            label_hidden.config(font=('Arial', 18))
            label_hidden.grid(pady=10)
            text_area = Text(d_f3, width=50, height=10)
            text_area.insert(INSERT, hidden_data)
            text_area.configure(state='disabled')
            text_area.grid()
            cancel_button = Button(d_f3, text='Cancel', command=lambda: self.page3(d_f3))
            cancel_button.config(font=('Arial', 12))
            cancel_button.grid(pady=15)
            info_button = Button(d_f3, text='More Info', command=self.info)
